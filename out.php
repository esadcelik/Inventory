...
<script type="text/javascript" src="control1.js"></script>
...
<div class="col-md-4">
<form action="outhandler.php" method="POST" autocomplete="off">
...
try {
	$STH = $DBH->prepare('SELECT kategori FROM stock Order by kategori ASC');  
	$STH->execute();
}
...
Nereye
<select class="form-control" id="nereye" name="nereye">
	<option selected></option>
...
<input class="form-control1" placeholder="Adet" type="text" id="adet" name="adet">
	...
	$date = date('Y-m-d');
	echo '<input class="form-control" type="text" id="cikis_tarih" name="cikis_tarih" value="'. htmlspecialchars($date, ENT_QUOTES) .'">';
	?>
	
<textarea class="form-control" placeholder="Açıklama" rows="2" name="aciklama" id="aciklama"></textarea>
<input class="btn btn-info btn-sm btn-block" type="submit" id="sub" name="submit" value="Çıkış Yap">
</form>
