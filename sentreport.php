...
<form action="reporthandler1.php" method="POST" autocomplete="off">
Kategori
...
try {
	$STH = $DBH->prepare("SELECT kategori FROM specifications");
	$STH->execute();
}
...
try {
	$STH = $DBH->prepare("SELECT isim FROM outlist");
	$STH->execute();  
}
catch(PDOException $e) {
    echo $e->getMessage();  
...
try {
	$STH = $DBH->prepare("SELECT nereye FROM outlist");  
	$STH->execute();  
}
catch(PDOException $e) {
    echo $e->getMessage();  
